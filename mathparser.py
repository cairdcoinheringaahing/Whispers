...
def juxtaposition(left, right):
    if not(isinstance(left, Variable) and isinstance(left.value, NUMBER)):
        return False
...
def parse(string, gettokens = True, red = True):
    ...
    types = (UnaryOp, BinaryOp, Variable, Function)
